...
class RegisterNMS(object):
    def __init__(
        ...
        # gs: onnx_graphsurgeon onnx图修改工具
        self.graph = gs.import_onnx(onnx.load(onnx_model_path)) # onnx模型导入onnx_graphsurgeon Graph
        assert self.graph
        LOGGER.info("ONNX graph created successfully")
        # Fold constants via ONNX-GS that PyTorch2ONNX may have missed 通过gs折叠py2onnx可能丢失的常量
