...
class ComputeLossOTA:
    ...
    def __call__(self, p, targets, imgs):  # predictions, targets, model   
        device = targets.device
        lcls, lbox, lobj = torch.zeros(1, device=device), torch.zeros(1, device=device), torch.zeros(1, device=device)
        bs, as_, gjs, gis, targets, anchors = self.build_targets(p, targets, imgs) # 为targets找到对应的候选框，找到对应GT的正样本bbox
    ...
    def find_3_positive(self, p, targets):
        ...
        for i in range(self.nl): # self.nl=3,有3个输出层
            anchors = self.anchors[i] # 当前输出层的 anchor
            gain[2:6] = torch.tensor(p[i].shape)[[3, 2, 3, 2]]  # 赋值，一会儿用，xyxy gain
            # p是预测结果，
            # Match targets to anchors
            t = targets * gain  # 归一化标签映射到特征图上
            if nt: # number of targets GT的数量
                # Matches
                r = t[:, :, 4:6] / anchors[:, None]  # wh ratio 每一个GT与anchor的宽高比大小
                j = torch.max(r, 1. / r).max(2)[0] < self.hyp['anchor_t']  # compare anchor比例不能大于self.hyp['anchor_t']倍，一般是4
                # j = wh_iou(anchors, t[:, 4:6]) > model.hyp['iou_t']  # iou(3,n)=wh_iou(anchors(3,2), gwh(n,2))
                t = t[j]  # filter

                # Offsets
                gxy = t[:, 2:4]  # 中心点到图片左上角(0,0)的距离 grid xy
                gxi = gain[[2, 3]] - gxy  # 中心点到图片右下角的距离 inverse
                j, k = ((gxy % 1. < g) & (gxy > 1.)).T # 离左上角近的选出来，而且不能是边界。gxy%1=不是整格的位置
