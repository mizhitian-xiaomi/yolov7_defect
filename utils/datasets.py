...
class LoadImages:  # for inference
    def __init__(self, path, img_size=640, stride=32):
        ...
        images = [x for x in files if x.split('.')[-1].lower() in img_formats] # 所有图片文件路径
        videos = [x for x in files if x.split('.')[-1].lower() in vid_formats] # 所有视频文件路径
        ni, nv = len(images), len(videos) # 图片、视频文件数量

        self.img_size = img_size
        self.stride = stride
        self.files = images + videos # 视频、图片文件
        self.nf = ni + nv  # number of files
        self.video_flag = [False] * ni + [True] * nv # 视频文件标志位
        self.mode = 'image' # 待处理的文件类型
        if any(videos):
    ...
    def __next__(self):
        if self.count == self.nf: # 停止迭代条件
            raise StopIteration
        path = self.files[self.count]

        if self.video_flag[self.count]:
            # Read video
            self.mode = 'video' # 修改处理文件类型
            ret_val, img0 = self.cap.read()
            if not ret_val:
                self.count += 1 # 文件+1
                self.cap.release()
                if self.count == self.nf:  # last video 停止迭代
                    raise StopIteration
                else:
                    path = self.files[self.count]
                    self.new_video(path)
        ...
        img = np.ascontiguousarray(img) # 上面的操作会导致内存地址不连续，调用ascontiguousarray使内存地址连续

        return path, img, img0, self.cap

    def new_video(self, path):
        self.frame = 0 # 读取视频的帧数
        self.cap = cv2.VideoCapture(path)
        self.nframes = int(self.cap.get(cv2.CAP_PROP_FRAME_COUNT)) # 当前视频文件总帧数
...
class LoadStreams:  # multiple IP or RTSP cameras
    def __init__(self, sources='streams.txt', img_size=640, stride=32):
        ...
        if os.path.isfile(sources):
            with open(sources, 'r') as f:
                sources = [x.strip() for x in f.read().strip().splitlines() if len(x.strip())] # 读取视频流地址
        else:
            sources = [sources]

        n = len(sources)
        self.imgs = [None] * n
        self.sources = [clean_str(x) for x in sources]  # 替换视频流地址特殊字符 clean source names for later
        for i, s in enumerate(sources):
            ...
            _, self.imgs[i] = cap.read()  # guarantee first frame imgs对应存储每个视频流的帧
            thread = Thread(target=self.update, args=([i, cap]), daemon=True) # 从视频流中取帧、跳帧
    ...
    def update(self, index, cap):
        ...
        while cap.isOpened():
            ...
            cap.grab()
            if n == 4:  # read every 4th frame 采集4帧解码一次
