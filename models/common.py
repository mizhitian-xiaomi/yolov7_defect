...
class SPPCSPC(nn.Module):
    ...
    def forward(self, x):
        ...
        y1 = self.cv6(self.cv5(torch.cat([x1] + [m(x1) for m in self.m], 1))) # 拼接4个maxpooling再卷积
        y2 = self.cv2(x)
...
class ImplicitA(nn.Module):
    ...
    def forward(self, x):
        return self.implicit + x # implicit类似偏置的感觉，对训练参数在加法层面上做平移的微调
...
class ImplicitM(nn.Module):
    ...
    def forward(self, x):
        return self.implicit * x    # 预测结果上做缩放微调
